...
- **Q:** The Challonge export design (reconciliation #13 + config) assumes the **classic API v1 static `CHALLONGE_API_KEY`**. Kevin has instead decided to use a **Challonge Connect OAuth2** app. The reconciliation's Challonge auth model needs updating. Resolved technical facts (verified from Challonge docs, 2026-08-16):
  ...
  - **Design impact:** the idempotency design (#13, `challonge_exports` table, create-vs-update, resumable steps) is unchanged; only the auth layer swaps from a static key to a cached OAuth token. **Ask for the design agent:** confirm this OAuth2-client-credentials model and update the reconciliation's Challonge section + config accordingly. The export MAPPING (K-Ball tournament JSON → v2 create tournament + bulk participants + matches/scores) is the next impl sub-slice and needs the v2 create/participants JSON:API payload shapes + the K-Ball tournament JSON schema.
